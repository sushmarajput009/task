...
public function test_form_input_data($data) {
    if (is_array($data)) {
        // If it's an array, apply trim recursively to each element
        return array_map(array($this, 'trim_recursive'), $data);
    } else {
        // If it's not an array, apply trim, stripslashes, and htmlspecialchars
        $data = trim($data);
...
private function trim_recursive($input) {
    if (is_array($input)) {
        // If it's an array, apply trim_recursive to each element
        return array_map(array($this, 'trim_recursive'), $input);
    } else {
        // If it's not an array, apply trim to the value
        return trim($input);
    }
}
...
/* -------------admin_password_change------------*/
	...
	public function add_new_task($data){
		...
		try {
			...
			// Iterate over the assigned users and execute the query for each
			foreach ($assign_to as $user_id) {
				$add_task->bindParam(':b', $user_id);
				$add_task->execute();
			}
